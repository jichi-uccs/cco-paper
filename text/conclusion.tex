\section{Conclusion}
\label{sec:cco:conclusion}

\todo { rewrite to make it appropriate for a paper}
\done{
In this paper, we present an integrated approach to optimize particular computation-communication patterns through overlapping.
We first find the potential time consuming communication loops through analytical performance modeling.
Then, we applied dependence analysis with domain knowledge and manually overlapped the computation and communication in the loop
  by shifting loop iterations.
Finally, we inserted MPI\_Test into computation loop and tuned the frequency of the MPI\_Test at runtime.
We applied our approach to NPB applications on both high-speed and low-speed inter-connected distributed environment and achieved up to 1.8x speedup.
}
